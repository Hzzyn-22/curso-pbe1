...
const manutencaoController = {
	...
	//Listar com Filtro Avancado ($gte / $lte em Custo)
	listarComFiltros: async (req, res) => {
		try{
			...
			const resultados = await Manutencao.find(query).sort({
				createdAt: -1 });
...
// Adicionar novo subdocumento de peça ao array pecasSubstituidas
adicionarPeca: async (req, res) => {
  try {
    const { id } = req.params;
    const novaPeca = req.body; // Dados do itemPecaSchema (nomePeca, quantidade, custoUnitario)
	...
	// Deletar Regitro por ID 
	excluir: async (req, res) => {
		try {
			const { id } = req.params;
			const removido = await Manutecao.findByIdAndDelete(id);
...
// Buscar por placa (busca parcial e case-insensitive)
buscarPorPlaca: async (req, res) => {
  try {
    const { placa } = req.params;

    // Utiliza $regex com a opção 'i' (case-insensitive)
